...
Decisions made: choose `mime-types` as the next public npm fixture because it is a small deterministic CommonJS lookup package with no fixture meta, native install, service, or stderr expectations.

## 2026-05-21 - MS public npm smoke

Shipped: extended the opt-in public npm smoke to the current `test-79-npm/ms` fixture. The fixture validates the package's CommonJS duration parser by checking `ms('15m')`.

Verified: target-node oracle probing passes for `ms` with `PKG_RUST_INSTALL_NPM_FIXTURES=1 PKG_RUST_REAL_CACHE=/private/tmp/pkg-rust-real-cache PKG_RUST_TARGET_ORACLE_PUBLIC_NPM=ms cargo test --locked --test runtime_smoke public_npm_target_node_oracle_probe_runs_when_enabled -- --nocapture`; the promoted packaged executable path passes with `PKG_RUST_INSTALL_NPM_FIXTURES=1 PKG_RUST_REAL_CACHE=/private/tmp/pkg-rust-real-cache cargo test --locked --test runtime_smoke public_npm_dictionary_fixtures_run_when_install_is_enabled -- --nocapture`; standard locked Rust gates pass (`cargo fmt --check`, `git diff --check`, `cargo check --locked --all-targets --all-features`, `cargo clippy --locked --all-targets --all-features -- -D warnings`, `cargo test --locked --all-targets --all-features`, `cargo test --locked --doc --all-features`, `RUSTDOCFLAGS=-Dwarnings cargo doc --locked --no-deps --all-features`, and `cargo bench --locked --bench packaging --no-run`).

Next: continue target-oracle-checking deterministic public npm fixtures before promotion, and keep deploy-file/native packages behind their separate gates.

Decisions made: choose `ms` because it is a small deterministic CommonJS utility package with no fixture meta, native install, service, or stderr expectations.

## 2026-05-21 - Underscore public npm smoke

Shipped: extended the opt-in public npm smoke to the current `test-79-npm/underscore` fixture. The fixture validates the package's CommonJS top-level export shape.

Verified: target-node oracle probing passes for `underscore` with `PKG_RUST_INSTALL_NPM_FIXTURES=1 PKG_RUST_REAL_CACHE=/private/tmp/pkg-rust-real-cache PKG_RUST_TARGET_ORACLE_PUBLIC_NPM=underscore cargo test --locked --test runtime_smoke public_npm_target_node_oracle_probe_runs_when_enabled -- --nocapture`; the promoted packaged executable path passes with `PKG_RUST_INSTALL_NPM_FIXTURES=1 PKG_RUST_REAL_CACHE=/private/tmp/pkg-rust-real-cache cargo test --locked --test runtime_smoke public_npm_dictionary_fixtures_run_when_install_is_enabled -- --nocapture`; standard locked Rust gates pass (`cargo fmt --check`, `git diff --check`, `cargo check --locked --all-targets --all-features`, `cargo clippy --locked --all-targets --all-features -- -D warnings`, `cargo test --locked --all-targets --all-features`, `cargo test --locked --doc --all-features`, `RUSTDOCFLAGS=-Dwarnings cargo doc --locked --no-deps --all-features`, and `cargo bench --locked --bench packaging --no-run`).
...
## 2026-05-21 - Json-stringify-safe 4 public npm smoke

Shipped: extended the opt-in public npm smoke to the pinned `test-79-npm/json-stringify-safe@4.0.0` wrapper. The wrapper reuses the circular JSON serialization body while installing the legacy package version.

Verified: target-node oracle probing passes for `json-stringify-safe@4.0.0` with `PKG_RUST_INSTALL_NPM_FIXTURES=1 PKG_RUST_REAL_CACHE=/private/tmp/pkg-rust-real-cache PKG_RUST_TARGET_ORACLE_PUBLIC_NPM='json-stringify-safe@4.0.0' cargo test --locked --test runtime_smoke public_npm_target_node_oracle_probe_runs_when_enabled -- --nocapture`; the promoted packaged executable path passes with `PKG_RUST_INSTALL_NPM_FIXTURES=1 PKG_RUST_REAL_CACHE=/private/tmp/pkg-rust-real-cache cargo test --locked --test runtime_smoke public_npm_dictionary_fixtures_run_when_install_is_enabled -- --nocapture`; standard locked Rust gates pass (`cargo fmt --check`, `git diff --check`, `cargo check --locked --all-targets --all-features`, `cargo clippy --locked --all-targets --all-features -- -D warnings`, `cargo test --locked --all-targets --all-features`, `cargo test --locked --doc --all-features`, `RUSTDOCFLAGS=-Dwarnings cargo doc --locked --no-deps --all-features`, and `cargo bench --locked --bench packaging --no-run`).
...
## 2026-05-21 - Extsprintf public npm smoke

Shipped: extended the opt-in public npm smoke to the current `test-79-npm/extsprintf` fixture. The fixture validates the package's CommonJS `sprintf` export shape.

Verified: target-node oracle probing passes for `extsprintf` with `PKG_RUST_INSTALL_NPM_FIXTURES=1 PKG_RUST_REAL_CACHE=/private/tmp/pkg-rust-real-cache PKG_RUST_TARGET_ORACLE_PUBLIC_NPM=extsprintf cargo test --locked --test runtime_smoke public_npm_target_node_oracle_probe_runs_when_enabled -- --nocapture`; the promoted packaged executable path passes with `PKG_RUST_INSTALL_NPM_FIXTURES=1 PKG_RUST_REAL_CACHE=/private/tmp/pkg-rust-real-cache cargo test --locked --test runtime_smoke public_npm_dictionary_fixtures_run_when_install_is_enabled -- --nocapture`; standard locked Rust gates pass (`cargo fmt --check`, `git diff --check`, `cargo check --locked --all-targets --all-features`, `cargo clippy --locked --all-targets --all-features -- -D warnings`, `cargo test --locked --all-targets --all-features`, `cargo test --locked --doc --all-features`, `RUSTDOCFLAGS=-Dwarnings cargo doc --locked --no-deps --all-features`, and `cargo bench --locked --bench packaging --no-run`).
